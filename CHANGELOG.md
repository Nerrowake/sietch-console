...
- **Multi-view UI redesign** — full redesign pass across Players, Metrics, Logs, and Diagnostics:
  - *Players*: replaces stock WPF `TabControl` with a custom pill-tab bar (Connected Players · Ban List · Allowlist); active pill highlighted with `SelectionBackground`; each list now has a column-header strip and `ItemContainerStyle` with hover and selection highlight; adds `SelectTabCommand(int)` to `PlayersViewModel`
  - *Metrics*: three stat tiles (Availability, Status, Downtime Events) with colour-coded 2 px top accent bars and large bold numbers; live indicator moved to the page header alongside the time-range `ComboBox`; Player Count chart annotated with a heuristic-accuracy note
  - *Logs*: filter buttons and action buttons extracted into named `FilterPillStyle` / `ToolbarActionStyle`; removes ~250 lines of duplicated inline `ControlTemplate` markup
